...
const (
	Apps        = "/namespaces/%s/apps"
	AppsHealth  = "/namespaces/%s/apps/%s/health"
	AppsMetrics = "/namespaces/%s/apps/%s/metrics"
)

func (c *Client) GetApps(namespace string) models.AppList {
	workloads := models.AppList{}
	request, err := http.NewRequest(http.MethodGet, c.GetRequestUrl(
		fmt.Sprintf(Apps, namespace), map[string]string{}), nil)
	if err != nil {
 ...
 **/
func (c *Client) GetAppsHealth(namespace, app, rateInterval, QueryTime, hType string) models.AppHealth {
	health := models.AppHealth{}
	request, err := http.NewRequest(http.MethodGet, c.GetRequestUrl(
		fmt.Sprintf(AppsHealth, namespace, app),
		map[string]string{
 ...
 **/
func (c *Client) GetAppsMetrics(namespace, app, avg, direction, duration, rateFunc, rateInterval,
	...
	request, err := http.NewRequest(http.MethodGet,
		c.GetRequestUrl(fmt.Sprintf(AppsMetrics, namespace, app),
			map[string]string{
				"avg":             avg,
				"direction":       direction,
				"duration":        duration,
